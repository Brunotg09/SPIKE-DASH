...
class _AuthFormState extends State<_AuthForm> with SingleTickerProviderStateMixin {
    ...
    return Column(
      ...
      children: [
        ...
        if (_isLogin) ...[
          ...
          Row(
            children: [
              Expanded(child: _buildSocialButton(label: 'Google', icon: Icons.g_mobiledata, onPressed: () {})),
              const SizedBox(width: 12),
              Expanded(child: _buildSocialButton(label: 'Apple', icon: Icons.apple, onPressed: () {})),
            ],
          ),
          const SizedBox(height: 16),
        ],
  ...
  Widget _buildSocialButton({required String label, required IconData icon, required VoidCallback onPressed}) {
    return OutlinedButton.icon(
      onPressed: onPressed,
      icon: Icon(icon, color: Colors.white, size: label == 'Google' ? 28 : 20),
